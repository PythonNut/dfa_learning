...
def break_dfa_symmetry_bfs_pop(M, V, sigma, prefixes_r, h):
    ...
    if s == 2:
        for j, i in it.combinations(range(1, h), 2):
            M.add_clause((-p[j, i], -p[j-1, i], y[1, i, j]))
    else:
        assert False
        m = V.dispense((s, h, h))
...
if __name__ == '__main__':
    train = read_dct('dcts/dfa_12_try_7.dct')
    ...
    while True:
        V = VariableDispenser()
        M = Maplesat(use_timer=True) # Glucose4(use_timer=True)

        print(f'Building problem for {states} states')
        min_dfa_setup_model_pop(M, V, train, prefixes_f, prefixes_r, G, sigma, states)
        break_dfa_symmetry_bfs_pop(M, V, sigma, prefixes_r, states)
    ...
    dfa = extract_dfa_pop(M, V, sigma, prefixes_r)
