...
set<int> set_state(int prefix,int state, int *prefix2state,unordered_map<int,set< int>> prefix_table, stack<int> prefix_table_p, stack<set<int>> prefix_table_t, bool trace);
set<int> update(int prefix,int state,int *prefix2state, set<int> different_set, unordered_map<int,set< int>> prefix_table, stack<int> prefix_table_p,stack<set<int>> prefix_table_t,set<int> potential, map< int,set< int>> prefix2constrain,vector<vector<int>> constrain_content, bool trace);
...
int main(){
	...
	*/
	...
	while(!fail){
		...
		stack<tuple< int,set< int>>> prefix_table_t;
		set<int> potential;
...
set<int> update(int prefix,int state,int *prefix2state, set<int> different_set, unordered_map<int,set< int>> prefix_table, stack<int> prefix_table_p,stack<set<int>> prefix_table_t,set<int> potential, map< int,set< int>> prefix2constrain,vector<vector<int>> constrain_content, bool trace){
	...
	for(auto k=p2c.begin();k!=p2c.end();k++){
		...
		int num_constrain = content.size()/3;
		int undetermined=-2;
		int result=0;
		...
		if(undetermined>-1 && result==num_constrain-1){
			...
			int s0=(d1!=0)?d1:d2;
			if(op==-1)
				set_state(p0,s0,prefix2state,prefix_table,prefix_table_p,prefix_table_t,trace);
			else{
				if(trace){
